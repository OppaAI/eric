...
SLAM changes (vs lidar.py):
  - init_lidar() shares the ROS2 node with odom.py and slam.py
  - /scan topic is consumed by both the safety callback AND SLAM Toolbox
  - No changes to safety logic — Layer 1 stops are unchanged
  - Node sharing priority: nav2 > odom > lidar (all reuse same node)
...
def init_lidar() -> bool:
    ...
    try:
        ...
        # Reuse existing ROS2 node — nav2 > odom > create new
        # All modules sharing one node means SLAM Toolbox, Nav2, and safety
        # callbacks all receive /scan without duplicate subscriptions.
        try:
            ...
            if nav2_node:
                ...
                raise Exception("nav2 node not ready")
        except Exception:
            try:
                from odom import _node as odom_node
                if odom_node:
                    _node = odom_node
                    log.info("LiDAR: reusing odom ROS2 node")
                else:
        ...
        # Only spin if not already spinning (nav2 or odom may be spinning already)
        try:
            ...
            pass
        try:
            from odom import _ros_thread as odom_thread
            if odom_thread and odom_thread.is_alive():
                log.info("LiDAR: ROS2 already spinning via odom")
                _lidar_ok   = True
                _lidar_stale = False
                _start_staleness_watchdog()
                _start_motor_heartbeat()
...
def _motors_stop(tag: str, reason: str):
    ...
    try:
        ...
        motors.stop()
        now  = time.monotonic()
        # Extract distance from reason string for change detection
        try:
