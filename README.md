...
- Cosmos parsing the SAR briefing into mission steps at startup, KV cache pre-filling before the first real call
- Pan-tilt 360° scan (`target_hunt` mode) — async inference overlapping with movement, per-position results collected in order
- Cosmos identifying a person lying on the floor as a casualty (`severity: CRITICAL`) from the camera feed alone
- `physical_reasoning` visible live in the Gradio log — judges watch Cosmos reason about the scene in real time
- `siren` alarm firing on confirmation — red LED strobe, rising audio tone, and TTS emergency broadcast
- `stay_with_target` protocol: Eric stays beside the casualty and repeats the location broadcast every 15 seconds
- Dual-camera photos saved automatically to `missions/photos/` on find
