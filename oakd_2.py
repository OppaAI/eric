...
Fixes applied (v3 → v4)
────────────────────────
1. ISP scale crash fix — the IMX378/214 sensor only supports 1080_P, 4_K, or
   12_MP. At 1080_P it outputs native 2104×1560 (not 1920×1080). setIspScale
   ratios don't land cleanly on standard sizes from this base. Fix: keep THE_1080_P
   and use setVideoSize(960, 540) to explicitly set the ISP output size. This
   avoids the post-proc mismatch and fits within OAK-D Lite SIPP memory budgets.
...
_depth_pub_node  = None    # ROS2 node for depth publishing
...
def _init_depth_publisher():
    """
    Create ROS2 publisher for /oakd/depth so Nav2 costmap can use
    OAK-D stereo depth for 3D obstacle awareness beyond what LiDAR sees.
    Reuses existing ROS2 node from nav2/odom/lidar if available.
    """
    global _depth_pub, _depth_pub_node
    try:
        import rclpy
        from sensor_msgs.msg import Image

        # Reuse existing node
        for mod_name in ("nav2", "odom", "lidar"):
            try:
                mod = __import__(mod_name)
                n = getattr(mod, "_node", None)
                if n:
                    _depth_pub_node = n
                    break
            except Exception:
        ...
        if _depth_pub_node is None:
            if not rclpy.ok():
                rclpy.init()
            _depth_pub_node = rclpy.create_node("eric_oakd_depth_pub")

        _depth_pub = _depth_pub_node.create_publisher(Image, "/oakd/depth", 10)
...
def _maybe_publish_depth(frame):
    ...
    try:
        ...
        msg              = Image()
        msg.header.stamp = _depth_pub_node.get_clock().now().to_msg()
