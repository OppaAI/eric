...
def launch():
    ...
    demo.launch(
        ...
        show_error=True,
        quiet=False,
        prevent_thread_lock=True,   # non-blocking — ROS2 executor owns main thread
        theme=gr.themes.Base(primary_hue="cyan", neutral_hue="slate"),
        css=_CSS,
    )
    # Block main thread here — ROS2 nodes spin in daemon threads
    # This keeps the process alive while all daemon threads run
    ...
    log.info("ERIC running — Ctrl+C to stop")
    stop_event.wait()